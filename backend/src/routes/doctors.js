...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const doctors = await prisma.doctor.findMany({ where: whereClause });

    // Inconsistent API formatting (directly sending array) - kept to not break frontend if it expects array
    res.json(doctors);
  } catch (error) {
...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    // Independent database calls are run concurrently with Promise.all
